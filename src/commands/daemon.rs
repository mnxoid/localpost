...
pub fn daemon(file: &str, key: &str) -> Result<()> {
    ...
    server.handle_connections(|request: &IPCRequest| handle_request(request, &mut state))?;
    Ok(())
}
...
fn handle_request(request: &IPCRequest, state: &mut DaemonState) -> IPCResponse {
    match request {
        IPCRequest::AddFile { key, path } => {
            if state.served_files.contains_key(path) {
